...
上面提到的 **不大建议在强事务性业务中使用本插件** ，注意有个 **'强'**，主要是为了避免大量数据保存的情况下，事务一次提交过多数据导致数据库压力过大，事务提交缓慢并长期占用数据库连接资源，
应用服务等待时间过长导致整体业务服务不稳定的现象。  
...
普通数据库访问方法跟批量模式下的操作都被一个事务管理着，要么一起成功要么一起失败。    
